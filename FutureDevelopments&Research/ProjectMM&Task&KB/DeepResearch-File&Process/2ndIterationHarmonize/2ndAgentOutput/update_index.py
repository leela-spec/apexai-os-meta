...
epic directories, task files, and handoff files, then produces a
Markdown registry under ``apex-meta/registry/index.md``.  It can run in
dry-run mode to preview the output without writing.

Changes in this hardened version:

- Includes the explicit relative file path for each task in the index table
  so that downstream checks do not have to infer paths from IDs or slugs.
- Uses only numeric task IDs (from front‑matter ``id`` or numeric
  filename); tasks lacking a numeric identifier are skipped.
- Preserves ``--dry-run`` behaviour; when dry-run is specified, the index
  is printed to stdout and no files are written.  Otherwise the file
  ``apex-meta/registry/index.md`` is replaced entirely with the new content.

The index groups pages into three types:
  * **Epics** – each slug directory under ``apex-meta/epics/``.  The slug name
    is listed along with the count of tasks it contains.
  * **Tasks** – each Markdown file under an epic directory.  For each task,
    the table includes the epic slug, task ID, title, status, priority,
...
def scan_repository(repo_root: str) -> Tuple[Dict[str, List[Dict[str, any]]], List[str]]:
    ...
    Each task record includes:
        ...
        slug: str
        filename: str
        rel_path: str (e.g., "epics/slug/filename.md")
    """
    ...
    if os.path.isdir(epics_root):
        for slug in sorted(os.listdir(epics_root)):
            ...
            for fname in sorted(os.listdir(slug_path)):
                ...
                # derive id from front‑matter or numeric filename; skip if not numeric
                tid = None
                if 'id' in fm:
                    try:
                        tid = int(fm['id'])
                ...
                if tid is None:
                    ...
                    if stem.isdigit():
                        tid = int(stem)
                if tid is None:
                    # skip tasks without a numeric identifier
                    print(
                        f"Warning: skipping task without numeric id: {path}",
                        file=sys.stderr,
                    )
                    continue
                ...
                rel_path = os.path.relpath(path, apex_meta)  # relative to apex-meta
                tasks.append(
                    {
                        'id': tid,
                        'title': title,
...
def build_index(epics: Dict[str, List[Dict[str, any]]], handoff: List[str]) -> str:
    ...
    if not any_tasks:
        ...
        lines.append('| Epic | ID | Title | Status | Priority | Path |')
        lines.append('| --- | --- | --- | --- | --- | --- |')
        for slug, tasks in epics.items():
            for task in tasks:
                tid = task['id']
                ...
                path = task['rel_path']
                lines.append(
                    f"| {slug} | {tid} | {title} | {status} | {priority} | {path} |"
                )
    lines.append('')
    # Handoff section
...
def write_index(repo_root: str, content: str) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Preview the generated index without writing to disk',
    )
