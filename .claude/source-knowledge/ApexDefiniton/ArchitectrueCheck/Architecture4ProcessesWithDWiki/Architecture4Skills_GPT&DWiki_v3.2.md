...
DeepWiki / Devin Prompt — Build One Hermes Skill First: Goal → Skeleton → Fill → Verify → Revise → Learn You are analyzing and working from the `NousResearch/hermes-agent` repository. I do **not**...

Hide full text# DeepWiki / Devin Prompt — Build One Hermes Skill First: Goal → Skeleton → Fill → Verify → Revise → Learn You are analyzing and working from the `NousResearch/hermes-agent` repository. I do **not** want a full process library yet. I want to build exactly **one** custom Hermes skill first: ```text ~/.hermes/skills/apex-process/goal-skeleton-fill-verify-revise-learn/SKILL.md ``` This skill should encode the general-purpose process: ```text Goal → Skeleton → Fill → Verify → Revise → Learn ``` The purpose is to give Hermes one reusable process for creating serious structured artifacts such as: * guides * decision memos * process definitions * prompt packs * research syntheses * skill drafts * implementation plans * Kanban graph specs * briefing packets Do **not** create Cron jobs. Do **not** create live Kanban tasks. Do **not** create multiple skills. Do **not** create a plugin/tool. Do **not** create profile files. Only create one implementation-ready `SKILL.md`. --- ## 1. Repo exemplars to inspect first Inspect these exact files and use them only as exemplars: ```text skills/research/research-paper-writing/SKILL.md skills/research/research-paper-writing/references/autoreason-methodology.md skills/software-development/plan/SKILL.md skills/software-development/requesting-code-review/SKILL.md optional-skills/software-development/subagent-driven-development/SKILL.md skills/devops/kanban-orchestrator/SKILL.md skills/devops/kanban-worker/SKILL.md website/docs/developer-guide/creating-skills.md website/docs/user-guide/features/skills.md ``` If a path differs in the current repo, search for the closest current file and report the actual path. --- ## 2. Required process definition The skill must implement this process: ```yaml process: name: goal-skeleton-fill-verify-revise-learn purpose: "Create a reliable structured artifact by defining the goal, building the skeleton first, filling content, verifying against acceptance criteria, revising, and capturing learning." phases: - goal_contract - skeleton_first - content_fill - verification - revision - learning_capture ``` ### Phase 1 — Goal contract The skill must make the agent define: ```yaml goal_contract: user_goal: artifact_type: audience: scope: non_scope: constraints: source_requirements: success_criteria: ambiguity_assumptions: stop_conditions: ``` ### Phase 2 — Skeleton first The skill must force a skeleton before content. The skeleton must include: ```yaml skeleton: artifact_title: section_list: - section_name: section_function: required_inputs: planned_output: acceptance_criteria: missing_inputs: risks: unresolved_questions: ``` Rule: do not fill the artifact until the skeleton is coherent. ### Phase 3 — Fill The fill phase must: ```yaml fill_rules: - fill one section at a time - preserve section function - avoid mixing evidence, decisions, open questions, and instructions in one block - make assumptions visible - keep unresolved issues visible - preserve source boundaries ``` ### Phase 4 — Verify The verification phase must generalize the verification logic from `requesting-code-review`, but for any artifact type. It must include: ```yaml verification_record: self_check: acceptance_criteria_check: missing_input_check: contradiction_check: source_grounding_check: completeness_check: independent_review_needed: pass_fail: required_fixes: ``` Decision rule: ```yaml verification_decision: pass: condition: "all critical acceptance criteria pass and no blocking contradictions remain" revise: condition: "fixable gaps exist" block: condition: "missing user decision, missing source, or contradiction blocks correctness" ``` ### Phase 5 — Revise The skill must define a revision loop: ```yaml revision_loop: max_cycles_default: 2 each_cycle: - list defects - prioritize critical defects first - patch skeleton if structure is wrong - patch content if content is wrong - rerun verification stop_when: - artifact passes - user decision is needed - max cycles reached ``` ### Phase 6 — Learn The skill must produce a learning capture section at the end: ```yaml learning_capture: reusable_pattern: failure_mode_seen: future_skill_candidate: memory_candidate: open_question: do_not_save: ``` This is not the same as automatically writing memory. The skill should only produce a structured candidate unless the user explicitly asks Hermes to save it. --- ## 3. Required SKILL.md format Follow the actual Hermes skill conventions from the repo. The output must be a full `SKILL.md` file with YAML frontmatter. Use this path: ```text ~/.hermes/skills/apex-process/goal-skeleton-fill-verify-revise-learn/SKILL.md ``` The skill must include: ```markdown --- name: description: version: author: license: platforms: metadata: hermes: tags: category: related_skills: --- # Goal Skeleton Fill Verify Revise Learn 2–3 sentence intro. ## When to Use ## Do Not Use For ## Required Inputs ## Quick Reference ## Procedure ## Output Format ## Verification ## Pitfalls ## Examples ## Related Hermes Translations ## Learning Capture ``` Use `requires_toolsets` only if the skill truly requires a toolset. If the skill can run as pure procedural reasoning, do not require toolsets. --- ## 4. Required adaptation report After the full skill, include this section: ```markdown # Adaptation Report ## What was adapted from research-paper-writing ## What was adapted from plan ## What was adapted from requesting-code-review ## What was adapted from subagent-driven-development ## What was adapted from kanban-orchestrator / kanban-worker ## What was deliberately not copied ## Why this skill should be one skill instead of five skills ## Where this skill may need to split later ``` --- ## 5. Required API-call analysis Estimate how many model/API calls this skill will cause in Hermes under different execution modes. Use this table: ```markdown | Mode | Assumptions | Model/API calls | Tool calls | Notes | |---|---|---:|---:|---| | Simple no-tool run | | | | | | With skill_view load | | | | | | With one verification pass | | | | | | With one revision cycle | | | | | | With independent reviewer via delegate_task | | | | | | With Kanban translation later | | | | | ``` Be explicit about what counts as: ```yaml counting_rules: model_api_call: "One chat/completions or equivalent model request." tool_call: "One Hermes tool invocation, not necessarily a model request." delegate_task_call: "Count parent tool call plus each spawned subagent's model calls separately." skill_view_call: "Count as a tool call; count model calls only if the agent needs another model turn after receiving skill content." background_review: "Mention separately if it may occur later; do not include unless triggered by the runtime." ``` If exact counts cannot be known from static repo inspection, provide: ```yaml api_call_estimate: minimum: typical: upper_bound: unknowns: files_to_check_for_exact_count: ``` --- ## 6. Quality bar The skill should be: * generic, not code-specific * generic, not ML-paper-specific * short enough to be usable * explicit enough that an agent can follow it * not overloaded with every possible process framework * compatible with later split-out skills such as `source-constraint-map`, `goal-recheck-validation`, and `diverge-synthesize` The main acceptance test: ```text Can Hermes use this one skill to create a structured artifact from a messy user request without immediately needing any other custom Apex skill? ``` Return only: 1. the full skill file, 2. the adaptation report, 3. the API-call analysis, 4. any blocking questions.
...
I need to read the exact frontmatter and section conventions from the key exemplar files before writing the skill. Let me pull them all in parallel.

Good. Now let me read the remaining exemplars in parallel — kanban-orchestrator, kanban-worker, and the key sections of research-paper-writing I need for the verification and learning capture patterns.

I now have everything I need. Here is the complete deliverable.
