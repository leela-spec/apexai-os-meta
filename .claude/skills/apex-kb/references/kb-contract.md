...
data_root_contract:
  ...
  required_directories:
    raw:
      path: "raw/"
      role: "Immutable copied, snapshotted, or explicitly pointer-preserved source intake."
      children: ["articles/", "papers/", "notes/", "refs/"]
    ingest_analysis:
    ...
    wiki:
      ...
      children: ["index.md", "concepts/", "entities/", "summaries/"]
    manifests:
      path: "manifests/"
      role: "Source manifest, hash records, and deterministic state metadata."
      required_files: ["source-manifest.json"]
    audit:
      path: "audit/"
    ...
    log:
      path: "log/"
      role: "Operation history, source-intake locks, and durable maintenance notes."
...
kb_schema_contract:
  ...
  role: "KB-local schema, authority, language, taxonomy, custody profile, and operator-review policy."
  required_fields:
    kb_topic_title: {type: string, required: true}
    kb_source_authority_list: {type: list, required: true}
    kb_concept_taxonomy_top_level: {type: list, required: true}
    kb_language_policy: {type: string_or_map, required: true}
    kb_operator_review_policy: {type: map, required: true}
  recommended_fields:
    kb_custody_profile:
      type: string
      allowed: [standard, skill_generation, research_base]
      purpose: "Determines source copy/snapshot strictness."
  must_not_contain:
...
source_policy:
  ...
  source_access_ledger_required_for_strict_profiles: true
  custody_profiles:
    standard:
  ...
  accepted_source_forms:
    local_markdown_or_text:
      standard_action: "Copy, snapshot, or preserve pointer according to custody profile."
      strict_action: "Snapshot/copy into raw/refs unless operator approves pointer exception."
    local_large_or_binary_file:
      standard_action: "Create pointer or copy/snapshot depending on size and operator decision."
      strict_action: "Snapshot/copy metadata and file when feasible; otherwise record pointer exception and access limits."
    url_or_external_reference:
      standard_action: "Store as source pointer unless operator supplies local content."
      strict_action: "Use only if local snapshot/copy exists or operator approves pointer exception."
    prior_kb_page:
  ...
  forbidden_source_behavior:
    ...
    - treat_manifest_entry_as_file_reading
...
wiki_page_contract:
  page_families:
    summaries:
      path: "wiki/summaries/"
      role: "Source-level or synthesis-level summaries grounded in preserved sources."
    concepts:
      path: "wiki/concepts/"
      role: "Reusable concept, framework, method, tool, or idea pages."
    entities:
      path: "wiki/entities/"
      role: "People, organizations, projects, products, repositories, or named systems."
    index:
      path: "wiki/index.md"
      role: "Primary navigation and query-entry surface."
  required_generated_page_properties:
    - stable_slug
    - clear_title
    - page_type
    - source_pointers
    - outbound_wikilinks
    - contradiction_or_uncertainty_section_when_needed
    - open_questions_when_needed
    - last_review_or_update_note
  page_generation_rules:
    preserve_source_pointers: true
    use_wikilinks_for_internal_links: true
    expose_contradictions: true
    expose_open_questions: true
    prefer_update_over_duplicate_page: true
    do_not_synthesize_without_source_basis: true
    do_not_claim_unread_sources: true
```
...
```yaml
index_contract:
  file: "apex-meta/kb/<kb-slug>/wiki/index.md"
  role: "Index-first query surface and compiled KB navigation layer."
  ownership_split:
    deterministic_structure:
      owner: python
      section_marker: "<!-- BEGIN AUTO-GENERATED INDEX -->"
      may_update:
        - page_file_list
    ...
    semantic_enrichment:
      owner: llm
      section_marker: "<!-- BEGIN LLM SUMMARY -->"
      may_update:
        - category_summaries
        - concept_descriptions
        - knowledge_gap_callouts
        - contradiction_summary
        - query_guidance_notes
  rules:
    query_reads_index_first: true
    generated_file_list_is_deterministic: true
    semantic_summary_must_not_claim_unread_sources: true
    stale_index_must_be_reported_before_query_confidence: true
```
...
operation_boundary_contract:
  scaffold:
    owns: [create_kb_root_structure, create_initial_kb_schema, create_initial_index, create_initial_manifest, create_initial_audit_and_log_paths]
    must_not: [ingest_sources_without_request, create_repo_global_CLAUDE_md, install_hooks_or_settings]
  source_intake_lock:
    owns:
      - accepted_source_universe
      - source_copy_or_snapshot_decisions
      - first_class_source_id_mapping
      - nested_source_promotion_or_defer_decisions
      - pointer_only_exceptions
      - observed_inventory
    must_not:
      - claim_semantic_understanding
      - create_wiki_pages
      - replace_phase_1_analysis
  ingest:
    owns:
      - phase_1_source_analysis
      - actual_file_read_ledger
      - operator_review_gate
      - phase_2_page_generation_after_approval
      - source_manifest_update
      - source_hash_record
      - contradiction_capture
      - crosslink_generation
    must_not:
      - skip_phase_1_when_review_required
      - generate_wiki_pages_before_operator_approval
      - silently_resolve_contradictions
      - treat_manifest_as_source_comprehension
  query:
    owns:
      - index_first_retrieval
      - selected_page_reading
      - wiki_grounded_answer
      - evidence_and_confidence_reporting
      - knowledge_gap_reporting
      - optional_saved_query_output
    must_not:
      - answer_from_raw_memory_when_relevant_pages_exist
      - read_entire_kb_when_3_to_5_pages_are_sufficient
  ...
  lint:
    owns:
      ...
      - semantic_review_flags_when_full_lint_requested
    must_not:
      - silently_rewrite_pages
      - treat_semantic_judgment_as_python_output
  audit:
    owns:
      - audit_file_listing
      - audit_grouping
      - target_anchor_review
      - accept_partial_reject_defer_options
      - resolved_audit_archival
    must_not:
      - resolve_human_feedback_without_operator_decision
...
```yaml
apex_package_boundary:
  apex_kb_owns:
    - durable_knowledge_base_compilation
    - source_intake_lock
...
```yaml
acceptance_contract:
  valid_kb_instance_requires:
    - kb_root_exists
    - kb_schema_md_exists
    - raw_tree_exists
    - ingest_analysis_folder_exists
    - wiki_index_exists
    - wiki_concepts_entities_summaries_exist
    - source_manifest_exists
    - audit_and_resolved_folders_exist
    - outputs_queries_folder_exists
    - log_folder_exists
  valid_source_intake_lock_requires:
    - accepted_sources_listed
    - excluded_or_deferred_sources_listed_or_none
    - storage_decisions_recorded
    - strict_pointer_exceptions_recorded_or_none
    - observed_inventory_recorded
    - nested_source_promotions_recorded_or_none
  valid_ingest_requires:
    - raw_source_or_approved_pointer_preserved
    - source_hash_or_no_hash_reason_recorded
    - phase_1_analysis_created
    - actual_file_read_ledger_present_when_required
    - operator_gate_respected
    - generated_pages_have_source_pointers
    - contradictions_or_uncertainties_flagged
    - index_updated_or_stale_index_flagged
    - manifest_updated_or_update_failure_reported
  valid_query_requires:
    - index_read_first
    - relevant_pages_selected
    - answer_grounded_in_compiled_pages
    - evidence_pages_named
    - contradictions_and_gaps_reported
    - confidence_stated
  valid_lint_requires:
    - deterministic_findings_separated_from_semantic_flags
    - broken_links_reported
    - orphan_pages_reported
    - stale_index_status_reported
    - malformed_frontmatter_reported
    - strict_profile_custody_issues_reported
  valid_audit_requires:
    - open_items_grouped
    - target_pages_identified
