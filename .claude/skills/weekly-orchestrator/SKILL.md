...
2. **Dispatch a stage.** Invoke the stage's subagent with a dispatch prompt containing exactly: the input packet paths, the operator's stage-specific intent, and any confirmed constraints. Do not paste packet contents — paths only (refs not copies).
3. **Receive the return.** A stage return is the handoff envelope + short summary. Validate the envelope against `references/handoff-schema.md` (all required fields present, `expected_action` and `stop_condition` non-empty). An invalid envelope goes back to the same agent once with the named gaps; a second failure halts the stage and reports.
