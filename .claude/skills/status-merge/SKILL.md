...
outputs:
  ...
  included_sections:
    ...
    - validation_status
  compatibility_note: >
    If older documents use updated_all_project_status_packet, interpret it here
    only as a proposal/view, never as accepted durable project state. Durable
    writes remain project-kb-manager owned and operator-gated.
```
...
5. **Draft proposal-only state views.** Produce `proposed_project_kb_update` and `updated_all_project_status_packet_proposal` as review/proposal views only. Durable project KB writes remain `project-kb-manager` owned and operator-gated.
...
failure_modes:
  ...
  - trigger: Legacy all-project status naming is used without proposal/view qualification.
    correction: Use updated_all_project_status_packet_proposal or an explicit compatibility note that says proposal/view only and not accepted durable state.
