...
learning_entry:
  id:
  status: candidate | strong_candidate | needs_validation | deferred | rejected | archived | integrated_into_scaffold | represented_in_appendix
  source_ref:
  summary:
...
## Approved deferred database and appendix candidates

| id | status | summary | candidate_target | evidence_refs | next action |
|---|---|---|---|---|---|
| LQ-INF-006 | deferred | Decide whether source/ranking/candidate ledgers need JSON/YAML sidecars for automated querying. | appendix or sidecar | `appendices/2Dos`, `KB_SYSTEM_RELIABILITY_AUDIT_V1` | Reopen only if Markdown tables prove insufficient for agent ingestion. |
| LQ-INF-007 | deferred | Decide whether a dedicated `APPENDIX_KB_SOURCE_NOTES.md` or source-notes database is needed. | appendix | `appendices/2Dos` | Locate actual SN surface or create a bounded source-notes appendix in a later pass. |
| LQ-INF-008 | deferred | Decide whether a separate `APPENDIX_KB_EXAMPLES.md` is needed with before/after transformations. | appendix | `appendices/2Dos` | Add only if future agents need examples to execute templates correctly. |
| LQ-INF-009 | represented_in_appendix | Maintain QA status, attach-pack guidance, and next research planning in `APPENDIX_KB_QA_AND_NEXT_RESEARCH_PLAN.md`. | appendix | `appendices/2Dos`, corrected promptflow | Use QA appendix as planning support, not accepted truth. |
...
- **Rule:** Approved but unresolved database-normalization items should move to explicit `deferred` status instead of becoming hidden TODOs.
...
- **Pointer:** `appendices/APPENDIX_KB_QA_AND_NEXT_RESEARCH_PLAN.md`
