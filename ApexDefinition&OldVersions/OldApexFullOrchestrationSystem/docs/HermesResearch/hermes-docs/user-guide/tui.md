...
URL Source: https://hermes-agent.nousresearch.com/docs/user-guide/tui
...
## Live session switcher[​](https://hermes-agent.nousresearch.com/docs/user-guide/tui#live-session-switcher "Direct link to Live session switcher")
...
![Image 1: Hermes TUI Session Orchestrator with one live session and a +new row](https://hermes-agent.nousresearch.com/img/docs/tui-session-orchestrator/session-orchestrator.png)

[Video 3](https://hermes-agent.nousresearch.com/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4)
Inside the switcher:
...
## Attaching to a running gateway[​](https://hermes-agent.nousresearch.com/docs/user-guide/tui#attaching-to-a-running-gateway "Direct link to Attaching to a running gateway")
...
`export HERMES_TUI_GATEWAY_URL="ws://localhost:8765/api/ws?token=<auth-token>"hermes --tui`

The token comes from the gateway's API auth configuration (see [API Server](https://hermes-agent.nousresearch.com/docs/user-guide/features/api-server)). When the env var is set, the TUI:
...
This is the same channel the web dashboard's embedded TUI uses (see [Web Dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard#chat)) — one gateway, many clients.
