...
URL Source: https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks
...
`# Startup Checklist1. Run `hermes cron list` and check if any scheduled jobs failed overnight.2. If any failed, send a summary to Discord #ops using the `send_message` tool.3. Check if `/opt/app/deploy.log` has any ERROR lines from the last 24 hours. If yes, summarize them and include in the same Discord message.4. If nothing went wrong, reply with only `[SILENT]` so no message is sent.`
...
*   Two hooks' return values affect behavior: [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_tool_call) can **block** the tool, and [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_llm_call) can **inject context** into the LLM call. All other hooks are fire-and-forget observers.

### Quick reference[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#quick-reference "Direct link to Quick reference")

| Hook | Fires when | Returns |
| --- | --- | --- |
| [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_tool_call) | Before any tool executes | `{"action": "block", "message": str}` to veto the call |
| [`post_tool_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_tool_call) | After any tool returns | ignored |
| [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_llm_call) | Once per turn, before the tool-calling loop | `{"context": str}` to prepend context to the user message |
| [`post_llm_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_llm_call) | Once per turn, after the tool-calling loop | ignored |
| [`on_session_start`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_start) | New session created (first turn only) | ignored |
| [`on_session_end`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_end) | Session ends | ignored |
| [`on_session_finalize`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_finalize) | CLI/gateway tears down an active session (flush, save, stats) | ignored |
| [`on_session_reset`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_reset) | Gateway swaps in a fresh session key (e.g. `/new`, `/reset`) | ignored |
| [`subagent_stop`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#subagent_stop) | A `delegate_task` child has exited | ignored |
| [`pre_gateway_dispatch`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_gateway_dispatch) | Gateway received a user message, before auth + dispatch | `{"action": "skip" | "rewrite" | "allow", ...}` to influence flow |
| [`pre_approval_request`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_approval_request) | Dangerous command needs user approval, before the prompt/notification is sent | ignored |
| [`post_approval_response`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_approval_response) | User responded to an approval prompt (or it timed out) | ignored |
| [`transform_tool_result`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_tool_result) | After any tool returns, before the result is handed back to the model | `str` to replace the result, `None` to leave unchanged |
| [`transform_terminal_output`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_terminal_output) | Inside the `terminal` tool, before truncation/ANSI-strip/redact | `str` to replace the raw output, `None` to leave unchanged |
| [`transform_llm_output`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_llm_output) | After the tool-calling loop completes, before the final response is delivered | `str` to replace the response text, `None`/empty to leave unchanged |
...
See the **[Build a Plugin guide](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
| Dimension | Shell hooks | [Plugin hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#plugin-hooks) | [Gateway hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#gateway-event-hooks) |
| --- | --- | --- | --- |
...
`// Block a pre_tool_call (both shapes accepted; normalised internally):{"decision": "block", "reason":  "Forbidden: rm -rf"}   // Claude-Code style{"action":   "block", "message": "Forbidden: rm -rf"}   // Hermes-canonical// Inject context for pre_llm_call:{"context": "Today is Friday, 2026-04-17"}// Silent no-op — any empty / non-matching output is fine:`
