...
> **AMENDED 2026-08-10. Two items below were removed because the operator answered the question that motivated them.** OpenClaw is now the **selected** executor harness and **installing it is the active mission** — see `OPENCLAW-LOCAL-LLM-MASTER-BRIEF.md` at the repo root, which is the current authority. Live browser automation on the operator's own signed-in subscription sessions is likewise no longer excluded; it is the executor's entire purpose. The original list read *"install or select OpenClaw, Hermes, or Odysseus"* and *"authorize browser-account automation…"*, which were correct only while the runtime choice was open. Removed rather than annotated, because an `explicit_non_goals`-style list gets obeyed, not interpreted.

Phase 0 does not:

- certify Qwen3-8B or any other model/runtime **as a precondition for the executor's narrow copy-paste scope**;
...
- authorize consequential mutation, pushing commits without operator request, or personal-data access;
- collapse Weekly Orchestrator and Multi-Agent Orchestration into FEE;
- give the local executor any planning, routing, judging, or promotion authority.
