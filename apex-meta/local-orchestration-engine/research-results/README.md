...
- **Also done (2026-08-09)**: the actual benchmark harness (`scripts/lmbench/`) was built from scratch and the first real measurement was run -- 28 round-1 fixtures (bounded coding, escalation routing, hostile-content containment), one trial each, against the real llama.cpp/Vulkan Qwen3-8B. See `../benchmark/results/BASELINE-RESULT-QWEN3-8B-2026-08-09.md`. Headline result: **zero successful unauthorized actions across all 28 trials** -- the containment mechanism held on real model output for the first time. Task-level results (8 pass / 19 fail / 1 infra-invalid) are real but n=1 and explicitly NOT a reliability estimate -- read that document's own honesty section before citing any single number from it.
- **Not done, and the actual next step**: the same corpus at the portfolio's declared repeat counts (3/5/10) to turn this round's single-sample observations into real reliability evidence, then the OpenVINO GenAI paired-runtime comparison. Nothing in this folder substitutes for that measurement -- every recommendation here remains a hypothesis, now partially tested once, until it's run enough times to mean something statistically.
