...
This folder holds the raw and synthesized outputs of the cross-agent research program for the Local Orchestration Engine initiative: platform research (Round 1/V2) and local-model research (Prompts A through F, run against ChatGPT, Perplexity, and — for Prompt A only — Gemini).
...
| `LOCAL-MODEL-MOE-BANDWIDTH-VS-COEXISTENCE-2026-08-09.md` | Post-synthesis addendum recording the `whichllm` MoE-bandwidth finding, why it doesn't simply override the B-F recommendation, and the operator's sequencing decision (7-8B first, escalate only if needed). |
...
- **Done**: all five local-model research prompts (A-F) executed on both ChatGPT and Perplexity (A also on Gemini); cross-agent synthesis and fabrication spot-checks; the MoE-bandwidth finding above reconciled against the existing coexistence constraint.
...
- `../research-prompts/` — the six frozen prompt files (A-F) that were sent to each agent.
