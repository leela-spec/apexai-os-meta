...
URL Source: https://hermes-agent.nousresearch.com/docs/user-guide/features/cron
...
Cron jobs use whatever provider `hermes model` selected. `hermes setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](https://hermes-agent.nousresearch.com/docs/integrations/nous-portal).
...
`# Pin a job to the `night-ops` profile regardless of where it was scheduledhermes cron create "every 1d at 03:00" \  "Tail the security log and flag anomalies" \  --profile night-ops`

`# From a chat, via the cronjob toolcronjob(    action="create",    schedule="every 1d at 03:00",    prompt="Tail the security log and flag anomalies",    profile="night-ops",)`
