...
URL Source: https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin
...
`def handle_scan(ctx, argstr):    """Implement /scan by invoking the terminal tool through the registry."""    result = ctx.dispatch_tool("terminal", {"command": f"find . -name '{argstr}'"})    return result  # returned to the caller's chat UIdef register(ctx):    ctx.register_command("scan", handle_scan, help="Find files matching a glob")`
...
### Lazy-install optional Python dependencies[​](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin#lazy-install-optional-python-dependencies "Direct link to Lazy-install optional Python dependencies")
...
### Overriding a built-in tool[​](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin#overriding-a-built-in-tool "Direct link to Overriding a built-in tool")
...
`# plugins/context_engine/my-engine/__init__.pyfrom agent.context_engine import ContextEngineclass MyContextEngine(ContextEngine):    @property    def name(self) -> str:        return "my-engine"    def should_compress(self, messages, model) -> bool: ...    def compress(self, messages, model) -> list[dict]: ...def register(ctx):    ctx.register_context_engine(MyContextEngine())`
