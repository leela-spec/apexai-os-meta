...
URL Source: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix
...
## How Hermes Behaves[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#how-hermes-behaves "Direct link to How Hermes Behaves")
...
### Session Model in Matrix[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#session-model-in-matrix "Direct link to Session Model in Matrix")
...
### Mention and Threading Configuration[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#mention-and-threading-configuration "Direct link to Mention and Threading Configuration")
...
`MATRIX_REQUIRE_MENTION=trueMATRIX_FREE_RESPONSE_ROOMS=!abc123:matrix.org,!def456:matrix.orgMATRIX_AUTO_THREAD=trueMATRIX_DM_MENTION_THREADS=falseMATRIX_REACTIONS=true          # default: true — emoji reactions during processing`
...
## Step 1: Create a Bot Account[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-1-create-a-bot-account "Direct link to Step 1: Create a Bot Account")

You need a Matrix user account for the bot. There are several ways to do this:

### Option A: Register on Your Homeserver (Recommended)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-a-register-on-your-homeserver-recommended "Direct link to Option A: Register on Your Homeserver (Recommended)")
...
### Option B: Use matrix.org or Another Public Homeserver[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-b-use-matrixorg-or-another-public-homeserver "Direct link to Option B: Use matrix.org or Another Public Homeserver")

1.   Go to [Element Web](https://app.element.io/) and create a new account.
2.   Pick a username for your bot (e.g., `hermes-bot`).

### Option C: Use Your Own Account[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-c-use-your-own-account "Direct link to Option C: Use Your Own Account")

You can also run Hermes as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-2-get-an-access-token "Direct link to Step 2: Get an Access Token")

Hermes needs an access token to authenticate with the homeserver. You have two options:

### Option A: Access Token (Recommended)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-a-access-token-recommended "Direct link to Option A: Access Token (Recommended)")
...
### Option B: Password Login[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-b-password-login "Direct link to Option B: Password Login")
...
## Step 3: Find Your Matrix User ID[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-3-find-your-matrix-user-id "Direct link to Step 3: Find Your Matrix User ID")
...
## Step 4: Configure Hermes Agent[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-4-configure-hermes-agent "Direct link to Step 4: Configure Hermes Agent")

### Option A: Interactive Setup (Recommended)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-a-interactive-setup-recommended "Direct link to Option A: Interactive Setup (Recommended)")
...
### Option B: Manual Configuration[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#option-b-manual-configuration "Direct link to Option B: Manual Configuration")

Add the following to your `~/.hermes/.env` file:

**Using an access token:**

`# RequiredMATRIX_HOMESERVER=https://matrix.example.orgMATRIX_ACCESS_TOKEN=***# Optional: user ID (auto-detected from token if omitted)# MATRIX_USER_ID=@hermes:matrix.example.org# Security: restrict who can interact with the botMATRIX_ALLOWED_USERS=@alice:matrix.example.org# Multiple allowed users (comma-separated)# MATRIX_ALLOWED_USERS=@alice:matrix.example.org,@bob:matrix.example.org`
...
### Start the Gateway[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#start-the-gateway "Direct link to Start the Gateway")
...
## End-to-End Encryption (E2EE)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#end-to-end-encryption-e2ee "Direct link to End-to-End Encryption (E2EE)")

Hermes supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.

### Requirements[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#requirements "Direct link to Requirements")

E2EE requires the `mautrix` library with encryption extras and the `libolm` C library:

`# Install mautrix with E2EE supportpip install 'mautrix[encryption]'# Or install with hermes extraspip install 'hermes-agent[matrix]'`
...
### Enable E2EE[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#enable-e2ee "Direct link to Enable E2EE")
...
### Cross-Signing Verification (Recommended)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#cross-signing-verification-recommended "Direct link to Cross-Signing Verification (Recommended)")
...
## Home Room[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#home-room "Direct link to Home Room")
...
### Using the Slash Command[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#using-the-slash-command "Direct link to Using the Slash Command")

Type `/sethome` in any Matrix room where the bot is present. That room becomes the home room.

### Manual Configuration[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#manual-configuration "Direct link to Manual Configuration")

Add this to your `~/.hermes/.env`:

`MATRIX_HOME_ROOM=!abc123def456:matrix.example.org`

## Room allowlist (`allowed_rooms`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#room-allowlist-allowed_rooms "Direct link to room-allowlist-allowed_rooms")
...
## Troubleshooting[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#troubleshooting "Direct link to Troubleshooting")

### Bot is not responding to messages[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#bot-is-not-responding-to-messages "Direct link to Bot is not responding to messages")
...
### Bot joins rooms but silently drops every message (clock skew)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#bot-joins-rooms-but-silently-drops-every-message-clock-skew "Direct link to Bot joins rooms but silently drops every message (clock skew)")
...
### "Failed to authenticate" / "whoami failed" on startup[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#failed-to-authenticate--whoami-failed-on-startup "Direct link to \"Failed to authenticate\" / \"whoami failed\" on startup")
...
### "mautrix not installed" error[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#mautrix-not-installed-error "Direct link to \"mautrix not installed\" error")
...
### Encryption errors / "could not decrypt event"[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#encryption-errors--could-not-decrypt-event "Direct link to Encryption errors / \"could not decrypt event\"")
...
### Upgrading from a previous version with E2EE[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#upgrading-from-a-previous-version-with-e2ee "Direct link to Upgrading from a previous version with E2EE")
...
## Proxy Mode (E2EE on macOS)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos "Direct link to Proxy Mode (E2EE on macOS)")
...
### How It Works[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#how-it-works "Direct link to How It Works")
...
### Step 1: Configure the Host (macOS)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-1-configure-the-host-macos "Direct link to Step 1: Configure the Host (macOS)")
...
### Step 2: Configure the Docker Container (Linux VM)[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-2-configure-the-docker-container-linux-vm "Direct link to Step 2: Configure the Docker Container (Linux VM)")
...
`FROM python:3.11-slimRUN apt-get update && apt-get install -y libolm-dev && rm -rf /var/lib/apt/lists/*RUN pip install 'hermes-agent[matrix]'CMD ["hermes", "gateway"]`

That's the entire container. No API keys for OpenRouter, Anthropic, or any inference provider.

### Step 3: Start Both[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#step-3-start-both "Direct link to Step 3: Start Both")
...
### Configuration Reference[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#configuration-reference "Direct link to Configuration Reference")
...
### Works for Any Platform[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#works-for-any-platform "Direct link to Works for Any Platform")
...
### Sync issues / bot falls behind[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#sync-issues--bot-falls-behind "Direct link to Sync issues / bot falls behind")
...
### Bot is offline[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#bot-is-offline "Direct link to Bot is offline")
...
### "User not allowed" / Bot ignores you[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#user-not-allowed--bot-ignores-you "Direct link to \"User not allowed\" / Bot ignores you")
...
## Security[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#security "Direct link to Security")
...
## Notes[​](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/matrix#notes "Direct link to Notes")
