...
this_file_rule_count: 17       # Descriptive: how many rules this guide contains
max_active_checks: 8
rule_count_ceilings:            # Research-backed hard ceilings for files being AUDITED
  standard_models: 50          # e.g. gpt-4o — safe plateau per IFScale
  reasoning_models: 100        # e.g. o3, gemini-2.5-pro — safely below 150-250 cliff onset
  always_loaded_context: 20    # prefer ≤20 when KB is loaded every turn
  per_project_configurable: true  # projects may set tighter limits in their own frontmatter
extended_check_count: 18
status: canonical
note_on_limits: >
  rule_count_ceilings are SYSTEM-WIDE hard ceilings grounded in IFScale research.
  Individual projects may configure tighter per-project limits (e.g. 12 or 20)
  in their own KB frontmatter as max_active_rules. Tighter is always safer.
  The ceiling in this guide is the MAXIMUM, not the recommended target.
owner: meta_ops
...
critical_rules:
  ...
  - id: BP-C2
    tier: critical
    rule: "Keep total rule count below 50 for standard deployment; stay below 100 for reasoning-class models — well under the 150–250 cliff onset observed in IFScale."
    prevents: "instruction-following cliff; 150–250 is the cliff ONSET for reasoning models — 100 is the safe ceiling; 50 is safe for standard models"
...
evidence_register:
  ...
  - id: F-01
    ...
    practical_implication: "Keep rule count below 50 for standard deployment; below 100 for reasoning models (the 150–250 range is the cliff onset, not the safe ceiling)."
...
extended_audit_checks:
  ...
  - id: CHK-03
    tier: critical
    check: "Total rule count is below 50 for standard models, below 100 for reasoning-class models."
    pass_condition: "rule_count < 50 (standard) OR rule_count < 100 (reasoning)"
    fail_action: halt
    evidence_source: F-01
