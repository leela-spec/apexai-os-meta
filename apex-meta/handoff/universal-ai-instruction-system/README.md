...
- A13 `<grounding>` owns **grounding-by-default and real-world verification**. For non-creative factual work whose correctness depends on information outside the supplied/local context, external grounding is the default rather than an exception: check the most specific authoritative evidence available for the actual task, environment, version, setting, method, or named system before relying on model reasoning. Prefer established primary/battle-tested sources and direct observation when the active environment itself is load-bearing. A08 governs evidence quality; C02 governs deeper research methodology.
