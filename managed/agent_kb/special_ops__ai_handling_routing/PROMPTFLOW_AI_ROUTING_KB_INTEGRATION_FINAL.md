...
Execute the actual AI Handling/Routing integration into the Apex AI OS repo.

This promptflow must integrate the newly placed AI Handling/Routing KB files into their standard paths while preserving the new base and patching only required Apex AI OS compatibility deltas.
...
## 11. AI Handling/Routing-specific warning
...
Skip the affected target and report if new base is missing after both folders checked, H1 missing or duplicated, identity ambiguous, old Apex logic conflicts with new base, patch would touch non-AI-Handling/Routing files, or patch would mutate runtime config/governance authority.

Stop the entire run if the target root is not `managed/agent_kb/special_ops__ai_handling_routing/` or repo is not `leela-spec/apexai-os-meta`.
