...
scope: hygiene_kb_lean_promotion
mode: newversion_base_plus_missing_apex_logic
---

# PROMPTFLOW — Hygiene KB Lean Promotion

## 0. Purpose

Promote the already-final Hygiene KB files from `newversion/` into the standard Hygiene KB paths while preserving only valid Apex AI OS logic that exists in the current standard files but is missing from the new files.
...
Your job is to make the new Hygiene KB files become the standard KB files with the smallest useful patch: add only missing Apex-specific operating logic from the old standard files.
...
hygiene routing into backlog, promotion, escalation, or validation lanes
```

Do not preserve:

```text
generic hygiene prose
...
- one compact bullet
- one small table row
- one explicit status/routing note
```
...
```text
appendices/APPENDIX_HYGIENE_KB_APEX_LOGIC_DELTA.md
```
...
patch would touch non-hygiene files
