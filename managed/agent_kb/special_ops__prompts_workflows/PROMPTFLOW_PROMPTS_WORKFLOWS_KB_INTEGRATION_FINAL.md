...
# PROMPTFLOW — special_ops__prompts_workflows Apex New-Base Delta Patch FINAL

## 0. Purpose

Patch the newly placed Prompts/Workflows KB files with missing Apex AI OS compatibility logic.
...
Skip the affected target and report if new base is missing after both folders checked, H1 missing or duplicated, identity ambiguous, old Apex logic conflicts with new base, patch would touch standard KB root files, patch would touch non-Prompts/Workflows files, or patch would mutate runtime config/governance authority.
