...
# PROMPTFLOW — special_ops__prompts_workflows Apex New-Base Integration FINAL

## 0. Purpose

Execute the actual Prompts/Workflows integration into the Apex AI OS repo.

This promptflow must integrate the newly placed Prompts/Workflows KB files into their standard paths while preserving the new base and patching only required Apex AI OS compatibility deltas.
...
The new base wins on structure, wording, headings, file roles, intentional overlap, cross-references, appendices, IDs, database tables, evidence links, ranking links, examples, templates, candidate links, and source lineage.
...
```text
managed/agent_kb/special_ops__prompts_workflows/newversion/
managed/agent_kb/special_ops__prompts_workflows/newversions/
```
...
Likely valid Prompts/Workflows deltas:

- MetaOps owns orchestration while Prompts/Workflows owns prompt construction
- Hygiene validates promptflow execution quality
- Knowledge Bank tracks candidate/promotion traces
- no direct config/runtime mutation constraints
...
Reject generic prompt prose, template cleanup, example compression, deduplication, appendix rewrites, external repo as active target, migration/process commentary, source/provenance sanitization unless exactly obsolete or Apex-incompatible, and future-research ideas unless already present as old Apex logic and required for Apex operation.
...
## 11. Prompts/Workflows-specific warning

Do not turn examples/templates into governance.

Do not strip repeated promptflow constraints. Templates may intentionally repeat rules from ESSENCE and BEST_PRACTICES.

Do not collapse prompt examples or workflow templates into summaries.

Appendices and conflict reports are database/evidence surfaces. Preserve IDs, row structures, ranking logic, candidate linkage, evidence linkage, source lineage, and cross-file pointers.
...
3. Preserve headings, order, overlap, examples, templates, IDs, tables, appendices, source lineage, conflict reports, and cross-references.
...
Skip the affected target and report if new base is missing after both folders checked, H1 missing or duplicated, identity ambiguous, old Apex logic conflicts with new base, patch would touch non-Prompts/Workflows files, or patch would mutate runtime config/governance authority.

Stop the entire run if the target root is not `managed/agent_kb/special_ops__prompts_workflows/` or repo is not `leela-spec/apexai-os-meta`.
