...
Reject structure normalization, schema improvements, deduplication, appendix rewrites, generic informatics prose, external repo as active target, migration/process commentary, source/provenance sanitization unless exactly obsolete or Apex-incompatible, and future-research ideas unless already present as old Apex logic and required for Apex operation.
...
Skip the affected target and report if new base is missing after both folders checked, H1 missing or duplicated, identity ambiguous, old Apex logic conflicts with new base, patch would touch standard KB root files, patch would touch non-Informatics files, or patch would mutate runtime config/governance authority.
