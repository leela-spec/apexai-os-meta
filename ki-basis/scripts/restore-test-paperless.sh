...
BACKUP_DIR_WIN="$(cygpath -m "$BACKUP_DIR" 2>/dev/null || echo "$BACKUP_DIR")"
...
TOKEN="${PAPERLESS_API_TOKEN:-$(getenv_file PAPERLESS_API_TOKEN)}"; [[ -n "$TOKEN" ]] || { echo "PAPERLESS_API_TOKEN required." >&2; exit 1; }
...
PG_IMAGE="$(docker inspect ki-basis-postgres --format '{{.Config.Image}}' | tr -d '\r')"; VK_IMAGE="$(docker inspect ki-basis-valkey --format '{{.Config.Image}}' | tr -d '\r')"; PL_IMAGE="$(docker inspect ki-basis-paperless --format '{{.Config.Image}}' | tr -d '\r')"; HELPER_IMAGE="$VK_IMAGE"
...
data = json.load(r)
matches = [x for x in data.get('results', []) if x.get('title') == title]
assert matches, f'Restored document not found: {title!r}'
doc_id = matches[0]['id']
req = urllib.request.Request(f'http://127.0.0.1:8000/api/documents/{doc_id}/download/', headers=headers)
