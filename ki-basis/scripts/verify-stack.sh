...
for c in ki-basis-postgres ki-basis-valkey ki-basis-firefly ki-basis-paperless ki-basis-openproject ki-basis-nginx ki-basis-hermes; do check_running "$c"; done
...
http_check(){ local label="$1" url="$2" code; code="$(curl -L -sS -m 15 -o /dev/null -w '%{http_code}' "$url" 2>/dev/null || true)"; [[ "$code" =~ ^[23] ]] && ok "$label HTTP $code" || fail "$label HTTP $code ($url)"; }
