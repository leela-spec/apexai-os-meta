#!/usr/bin/env python3
"""Extract final apex-sync variant files from the ProThinking full response.

The extractor copies file-body bytes exactly from fenced blocks. It supports
both triple-backtick and quadruple-backtick fences because the Markdown package
files contain nested fenced blocks.
"""
...
SOURCE_PATH = (
    ...
    / "Prothinking_Apex_synch_fullresponse.md"
)
REPORT_PATH = REPO_ROOT / ".claude" / "skills" / "apex-sync" / "apex-sync-final-extraction-report.md"
...
def resolve_target(raw_target: bytes) -> Path:
    ...
    apex_sync_root = REPO_ROOT / ".claude" / "skills" / "apex-sync"
    scripts_root = REPO_ROOT / "scripts"
    if not (target.is_relative_to(apex_sync_root) or target.is_relative_to(scripts_root)):
        raise ValueError(f"Target is outside allowed apex-sync paths: {target_text}")
    if target.suffix not in {".md", ".py"}:
        raise ValueError(f"Target must be Markdown or Python: {target_text}")
    return target
...
def find_fenced_content(data: bytes, search_from: int) -> tuple[int, int, int, bytes]:
    opener = re.search(br"(?P<fence>`{3,})(?:[A-Za-z0-9_-]+)?[ \t]*\r?\n", data[search_from:])
    if not opener:
        raise ValueError("Missing opening fenced block")
    ...
    while line_start < len(data):
        ...
        if line_without_cr.strip() == fence:
            ...
            if content_end > content_start and data[content_end - 1 : content_end] == b"\n":
                ...
                if content_end > content_start and data[content_end - 1 : content_end] == b"\r":
                    content_end -= 1
            return content_start, content_end, next_line_start, fence

        line_start = next_line_start

    raise ValueError("Missing closing fenced block")
...
def parse_blocks() -> list[Block]:
    ...
    heading_re = re.compile(
        br"(?m)^#{2,4}[ \t]+(?:\d+(?:\.\d+)?[ \t]+)?"
        br"`(?P<target>(?:\.claude/skills/apex-sync/|scripts/)[^`]+)`[ \t]*\r?\n[ \t]*\r?\n"
    )
    blocks: list[Block] = []

    for match in heading_re.finditer(data):
        target_path = resolve_target(match.group("target"))
        content_start, content_end, _after, fence = find_fenced_content(data, match.end())
...
def render_report(blocks: list[Block], actions: list[Action]) -> bytes:
    ...
    for block in blocks:
        ...
        lines.append(f"  - fence: `{block.fence.decode('ascii')}`")
